...
PHPStan map files, dynamic return type extensions, source-derived structural
stubs, and a classmap autoloader.
...
Structural stubs are generated by parsing Magento source files with
`nikic/php-parser`; Magento is not bootstrapped and source files are not
executed.
...
It extracts declared classes and interfaces, inheritance, implemented
interfaces, public/protected methods, parameters, defaults, native return types,
and method PHPDoc. The parsed output is split into:

```text
mage-factories.stub.php
magento-core.stub.php
varien.stub.php
```

A small set of method-level overrides is still applied where Magento's source
signature is too weak for PHPStan, such as factory methods and selected
`Varien_Object` data methods. These overrides refine parsed methods; full
framework classes are not maintained as hand-written templates.
...
- Structural stubs are source-derived, with only method-level PHPStan
  refinements kept in code.
